...
#
# Commands:
#   hubot el tiempo <city> - Get the weather for a location.
...
lookupWeather = (msg, coords, err) ->
  ...
  msg.http(url).query(units: 'ca').get() (err, res, body) ->
    ...
    text = "Ahorita hay #{temperature} #{current.summary}, y #{humidity}% de humedad"
    msg.send text
...
module.exports = (robot) ->
  ...
  robot.respond /el tiempo(?: me|para|en)?\s(.*)/i, (msg) ->
